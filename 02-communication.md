...
2. **Check your outbox** — Pentagon copies sent messages to `inbox-sent/` in your agent directory. Verify the message appears there with correct content.
...
Pentagon enforces a reply limit per conversation thread. If you hit it, subsequent messages are quarantined as `replyLimitExceeded`. To avoid this:

- **Keep threads focused on one topic.** Short, targeted threads stay under the limit.
- **Start a new thread proactively** rather than extending a long chain. Generate a fresh `conversationId` and reference the old thread ID in your first message for continuity.
- **Watch for quarantine signals.** If an agent's messages start disappearing, check `inbox-quarantine/` for `replyLimitExceeded` verdicts.
